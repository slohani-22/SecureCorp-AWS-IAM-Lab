...
!\[SecureCorp AWS IAM Lab Architecture](screenshots/architecture-diagram.png)
...
```python
import boto3, json
client = boto3.client('secretsmanager', region\_name='us-east-1')
response = client.get\_secret\_value(SecretId='securecorp/db/password')
...
* AWSReservedSSO\_SecureCorp-ReadOnly — intentional IAM Identity Center SAML federation role
...
|iam\_users.tf|Users, groups, memberships, all IAM policies, password policy|
